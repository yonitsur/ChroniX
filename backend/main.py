...
from fastapi import FastAPI, HTTPException, Header, Body, Depends, Request, Response, status, File, Form, UploadFile
...
async def get_guest_policy():
    ...
    and its configured limits) so the pre-login screen can render accurate copy before
    any guest session/user object exists.
    """
    mode = get_guest_api_key_mode()
    return {
        "mode": mode,
        "daily_paid_limit": get_guest_daily_limit() if mode == "limited" else -1,
        "guest_daily_chat_limit": get_guest_daily_chat_limit() if mode == "limited" else -1,
    }
...
async def chat_attach_files_endpoint(
    ...
    try:
        ...
        logger.info(
            ...
            f"({len(file_payloads)} files, remaining_refines={remaining_refines})"
        )
        ...
        usage = get_timeline_ai_usage(timeline_id)
        updated_timeline.aiRefineCount = usage.get("refine_count", (timeline_data.aiRefineCount or 0) + 1)
        updated_timeline.aiEventAddCount = usage.get("event_add_count", timeline_data.aiEventAddCount or 0)
        user_id = user.get("id")
        await asyncio.to_thread(save_timeline_data, updated_timeline.model_dump(by_alias=True), user_id=user_id)
...
async def set_timeline_share_status(
    ...
    """Owner-only: enable/disable the shareable "anyone with the link" flag."""
    enabled = bool(body.get("enabled", True))
    meta = get_timeline_with_meta(timeline_id)
    if not meta or not meta.get("data"):
        raise HTTPException(status_code=404, detail="Timeline not found")
    if meta.get("owner_id") != user.get("id"):
        raise HTTPException(status_code=403, detail="Only the owner can change sharing for this timeline")

    success = set_timeline_shared(timeline_id, user.get("id"), enabled)
    if not success:
