...
def test_timeline_pin_serialization_and_saved_list(tmp_path, monkeypatch):
    ...
    timeline = TimelineData(
        ...
        title="Pinned",
        isPinned=True,
        articles=[TimelineArticle(id="a1", title="Ev", **{"from": TimelineDate(year=2000)})]
    )
...
async def test_multilingual_batch_enrichment_spanish_and_fallback():
    ...
    # Second event: the fake local title is bridged via the (redirected) English title's
    # interlanguage link to the SAME entity's Spanish article.
    assert "es.wikipedia.org" in enriched[1].get("wikiUrl", "")
    assert "Independencia" in enriched[1].get("wikiTitle", "")
    assert enriched[1].get("imageUrl") is not None
...
def test_chat_starters_models_and_serialization():
    from models import GeminiTimelineOutput
    output = GeminiTimelineOutput(
        title="The Roman Republic",
        description="Rise and fall of Rome",
        overview="Narrative overview of Rome.",
        chat_questions=["Why did the Gracchi reforms fail?"],
        chat_edit_ideas=["Add the key battles of the Punic Wars", "Remove the minor consuls"],
    )
    tl = TimelineData(
        id="tl-1",
        title=output.title,
        chatQuestions=output.chat_questions,
        chatEditIdeas=output.chat_edit_ideas,
    )
    dumped = tl.model_dump(by_alias=True)
    assert dumped["chatQuestions"] == ["Why did the Gracchi reforms fail?"]
    assert dumped["chatEditIdeas"][0] == "Add the key battles of the Punic Wars"

    legacy = TimelineData.model_validate({"id": "tl-2", "title": "Old timeline"})
    assert legacy.chatQuestions == [] and legacy.chatEditIdeas == []
...
@pytest.mark.asyncio
async def test_wikipedia_summary_retries_rate_limit(monkeypatch):
    from services import wiki_enricher
    ...
    responses = [
        ...
        httpx.Response(
            ...
            json={
                "title": "Apollo 11",
                "description": "American spaceflight",
                "extract": "Apollo 11 was a spaceflight.",
    ...
    monkeypatch.setattr(wiki_enricher.asyncio, "sleep", fake_sleep)

    result = await wiki_enricher.fetch_wikipedia_summary(
        "Apollo 11", FakeClient(), asyncio.Semaphore(1)
    )

    assert result["wikiTitle"] == "Apollo 11"
    assert len(calls) == 2
    assert sleeps == [0.5]


@pytest.mark.asyncio
async def test_wikipedia_search_retries_transport_failure(monkeypatch):
    from services import wiki_enricher
    ...
    class FakeClient:
        ...
        async def get(self, *args, **kwargs):
            ...
            return httpx.Response(200, json={"query": {"search": [{"title": "Apollo 11"}]}})
    ...
    titles = await wiki_enricher._search_wikipedia_titles("Apollo 11", FakeClient())

    assert titles == ["Apollo 11"]
    assert sleeps == [0.3]


@pytest.mark.asyncio
async def test_narrative_anchors_ranking_boost():
    """Verify that finalize_timeline_from_parsed boosts the earliest and latest events to rank 10."""
    from unittest.mock import AsyncMock, patch
    from models import GeminiTimelineOutput, GeminiLaneItem, GeminiEventItem
    from services.gemini_service import finalize_timeline_from_parsed

    parsed = GeminiTimelineOutput(
        title="Test Timeline",
        description="Testing narrative anchors",
        overview="Overview...",
        lanes=[GeminiLaneItem(id="main", title="Main", color="#2b5278")],
        events=[
            GeminiEventItem(
                id="mid",
                title="Middle Event",
    ...
    with patch("services.gemini_service.enrich_events_with_wikipedia", new=AsyncMock(side_effect=lambda evs, **kw: evs)):
        timeline_data = await finalize_timeline_from_parsed(parsed, subject_label="Test")

    first_ev = next(a for a in timeline_data.articles if a.id == "first")
    mid_ev = next(a for a in timeline_data.articles if a.id == "mid")
    last_ev = next(a for a in timeline_data.articles if a.id == "last")

    assert first_ev.rank == 10
    assert last_ev.rank == 10
    assert mid_ev.rank == 5


def test_save_timeline_data_rejects_empty_articles(tmp_path, monkeypatch):
    from services import storage
    monkeypatch.setattr(storage, "TIMELINES_FILE", tmp_path / "timelines.json")
    monkeypatch.setattr(storage, "_supabase_is_configured", lambda: False)

    # Empty articles
    empty_timeline = {"id": "empty-tl", "title": "Empty", "articles": []}
    storage.save_timeline_data(empty_timeline)
    saved = storage.list_all_timelines()
    assert len(saved) == 0
    ...
    valid_timeline = {
        "id": "valid-tl",
        "title": "Valid",
        "articles": [{"id": "ev-1", "title": "Event 1", "from": {"year": 2020}}]
    }
    storage.save_timeline_data(valid_timeline)
    saved_after = storage.list_all_timelines()
    assert len(saved_after) == 1
    assert saved_after[0]["id"] == "valid-tl"


@pytest.mark.asyncio
async def test_finalize_timeline_from_parsed_rejects_empty_events():
    from services.gemini_service import finalize_timeline_from_parsed
    from models import GeminiTimelineOutput

    empty_parsed = GeminiTimelineOutput(
        title="Empty",
        description="Empty test",
        overview="Overview text",
        events=[]
    )
    with pytest.raises(ValueError, match="returned 0 events"):
        await finalize_timeline_from_parsed(empty_parsed, subject_label="Test")
