...
def set_timeline_shared(timeline_id: str, user_id: str, enabled: bool) -> bool:
    """
    ...
    if not _supabase_is_configured():
        # Local dev fallback: timelines are already treated as always-shared.
        return True
    try:
        url = f"{SUPABASE_URL}/rest/v1/timelines"
        headers = _get_supabase_headers()
        headers["Prefer"] = "return=minimal"
        params = {"id": f"eq.{timeline_id}", "user_id": f"eq.{user_id}"}
        with httpx.Client(timeout=5.0) as client:
            resp = client.patch(url, headers=headers, params=params, json={"is_public": enabled})
            if resp.status_code in (200, 204):
                return True
            logger.warning(f"Supabase set_timeline_shared status {resp.status_code}: {resp.text}")
    except Exception as e:
        logger.warning(f"Failed to update timeline share flag: {e}")
    return False
