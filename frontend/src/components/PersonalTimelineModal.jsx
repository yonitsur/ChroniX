      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-line bg-surface-raised">
          ...
          <button
            ...
            className="text-ink-subtle hover:text-ink p-1 rounded-control hover:bg-surface-hover transition-colors shrink-0 disabled:opacity-40 disabled:cursor-not-allowed"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
          ...
          <div>
            <label className="flex items-center gap-1.5 text-xs font-medium text-ink mb-1.5">
              ...
              {isAiMode && <span className="text-[10px] font-normal text-ink-subtle">({t('common.optional')})</span>}
            </label>
            <input
              type="text"
              value={title}
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <button
                ...
                disabled={isGeneratingFromFiles}
                className={`flex flex-col items-start gap-1 p-3 rounded-panel border text-start transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${
                  isUploadMode
              ...
              {isUploadMode ? (
                <>
                  ...
                  <div
                    ...
                    className={`flex flex-col items-center justify-center gap-1.5 p-5 rounded-panel border-2 border-dashed text-center cursor-pointer transition-colors ${
                      isDragOver ? 'border-accent bg-accent-soft' : 'border-line hover:border-line-strong bg-surface-sunken'
                  ...
                  >
                    ...
                    <p className="text-[11px] text-ink-muted">{t('uploadTimeline.dropzoneHint')}</p>
                    <input
                      ref={fileInputRef}
                      type="file"
                      multiple
                  ...
                  {files.length > 0 && (
                    <div>
                      <div className="flex items-center justify-between mb-1.5">
                        ...
                        <span className="text-[11px] text-ink-subtle">{formatBytes(totalUploadBytes)}</span>
                      </div>
                      <div className="space-y-1 max-h-40 overflow-y-auto pe-1">
                        {files.map((f, idx) => (
                          ...
                          >
                            ...
                            <span className="text-[10px] text-ink-subtle shrink-0">{formatBytes(f.size)}</span>
                            <button
                              type="button"
        ...
        <div className="flex items-center justify-end gap-2 px-6 py-3.5 border-t border-line bg-surface-raised">
          <button
            ...
            onClick={onClose}
            disabled={isGeneratingFromFiles}
            className="px-4 py-2 text-sm font-medium text-ink-muted hover:text-ink rounded-control hover:bg-surface-hover transition-colors cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed"
          >
            {t('common.cancel')}
          </button>
          <button
            ...
            className="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-semibold text-accent-fg bg-accent hover:bg-accent-hover rounded-control shadow-control transition-colors cursor-pointer active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed"
          >
