...
export default function FloatingMapWidget({
  ...
  const [pipSize, setPipSize] = useState({ width: 420, height: 290 });
  ...
  useEffect(() => {
    setIconPos(null);
    setPipPos(null);
    setPipSize({ width: 420, height: 290 });
  }, [timelineId]);
    ...
    const handleResize = () => {
      if (iconRef.current && iconPos) {
        ...
        setIconPos({
          ...
          y: Math.max(10, Math.min(window.innerHeight - rect.height - 10, iconPos.y))
        });
      }
    ...
    return (
      ...
      >
        ...
        >
          ...
          {geoCount > 0 && (
            <span className="flex items-center gap-0.5 bg-surface-sunken text-ink text-caption font-semibold px-1.5 py-0.5 rounded-full border border-line pointer-events-none">
