...
export default function AuthGate({ theme, onToggleTheme }) {
  ...
  const [guestLimits, setGuestLimits] = useState({ mode: 'limited', timelines: 5, chats: 15 });
    ...
    fetchGuestPolicy()
      .then((p) => {
        if (isMounted && p) {
          setGuestLimits({
            mode: p.mode || 'limited',
            timelines: p.daily_paid_limit > 0 ? p.daily_paid_limit : 5,
            chats: p.guest_daily_chat_limit > 0 ? p.guest_daily_chat_limit : 15,
          });
        }
      })
      .catch(() => {});
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="mt-2 text-[11px] text-ink-subtle text-center leading-relaxed">
                        {guestLimits.mode === 'free'
                          ? t('auth.guestCaptionFree')
                          : guestLimits.mode === 'unlimited'
