...
import OpenSourceView from './public/OpenSourceView';
import AmbientAuraBackground from './AmbientAuraBackground';
import { fetchGuestPolicy } from '../api';
import { getDefaultQuotaPolicy, getGuestCaptionText } from '../utils/quotaPolicy';
...
export default function AuthGate({ theme, onToggleTheme }) {
  ...
  const [guestLimits, setGuestLimits] = useState(() => {
    ...
    return {
      mode: defaults.guestMode,
      limit: defaults.guestLimit,
      guestMode: defaults.guestMode,
      guestLimit: defaults.guestLimit,
      registeredMode: defaults.registeredMode,
      registeredLimit: defaults.registeredLimit,
    };
  });
    ...
    fetchGuestPolicy()
      .then((p) => {
        if (isMounted && p) {
          setGuestLimits((prev) => ({
            ...prev,
            mode: p.guest_mode || p.mode || prev.mode,
            limit: p.guest_daily_limit > 0 ? p.guest_daily_limit : (p.daily_paid_limit > 0 ? p.daily_paid_limit : prev.limit),
            guestMode: p.guest_mode || p.mode || prev.guestMode,
            guestLimit: p.guest_daily_limit > 0 ? p.guest_daily_limit : (p.daily_paid_limit > 0 ? p.daily_paid_limit : prev.guestLimit),
            registeredMode: p.registered_mode || prev.registeredMode,
            registeredLimit: p.registered_daily_limit > 0 ? p.registered_daily_limit : prev.registeredLimit,
          }));
        }
      })
      .catch(() => { });
  ...
  const [currentView, setCurrentView] = useState(() => {
    if (typeof window !== 'undefined') {
      ...
      if (path.includes('open-source') || hash.includes('open-source') || path.includes('opensource') || hash.includes('opensource')) return 'open-source';
  ...
  useEffect(() => {
    const handleUrlChange = () => {
      ...
      else if (path.includes('open-source') || hash.includes('open-source') || path.includes('opensource') || hash.includes('opensource')) setCurrentView('open-source');
  ...
  useEffect(() => {
    const titleMap = {
      ...
      home: 'ChroniX - Interactive Visual Chronologies',
    };
  ...
  return (
    ...
    >
      ...
      <div className="public-page-wrapper flex-1">
                    ...
                    {supportedLanguages.map((l) => {
                      ...
                      return (
                        <button
                          ...
                          className={`w-full px-3 py-1.5 text-xs flex items-center justify-between transition-colors cursor-pointer text-start ${isSelected
        ...
        {currentView === 'terms' && (
          <TermsView
            onOpenOpenSource={() => navigateTo('open-source')}
            onOpenPublicRoute={navigateTo}
          />
        )}

        {currentView === 'open-source' && (
          <OpenSourceView onBackToTerms={() => navigateTo('terms')} />
        )}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-14 items-start">
                ...
                <div className={`lg:col-span-7 flex flex-col ${isRtl ? 'text-right' : 'text-left'}`} dir={isRtl ? 'rtl' : 'ltr'}>
                  <div className="flex flex-wrap items-center gap-3.5 mb-6">
                    <div className="inline-flex items-center gap-2 text-ink-muted font-medium text-sm sm:text-base">
                      <Sparkles className="w-4 h-4 text-ink-subtle shrink-0" />
                      <span>{t('auth.landingHeroBadge')}</span>
                    </div>
                    <span className="text-ink-faint text-sm">•</span>
                    <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-success-soft border border-success/25 transition-colors">
                      <Heart className="w-3.5 h-3.5 text-success fill-success/20 shrink-0" />
                  ...
                  <h1 className="text-4xl sm:text-6xl lg:text-7xl font-semibold text-ink tracking-tighter leading-[1.06] mb-6 sm:mb-7">
                    {t('auth.landingHeroTitle')}
                  </h1>

                  <p className="text-lg sm:text-xl md:text-2xl text-ink-muted leading-relaxed mb-6 max-w-2xl">
                ...
                <div className="lg:col-span-5 flex justify-center w-full">
                  <div
                    id="auth-card"
                    className="w-full max-w-md bg-surface-raised/85 backdrop-blur-2xl border border-line rounded-sheet p-5 sm:p-8 shadow-panel transition-colors"
                    dir={isRtl ? 'rtl' : 'ltr'}
                  >
                    {/* Card Header */}
                    ...
                    <div className="mb-4">
                      <button
                        ...
                        className="w-full flex items-center justify-center gap-3 px-4 py-3 bg-surface-raised hover:bg-surface-hover text-ink font-medium text-sm rounded-control border border-line-strong shadow-control transition-colors active:scale-[0.98] disabled:opacity-50 cursor-pointer"
                      >
                    ...
                    <div className="relative flex items-center justify-center mb-4">
                      ...
                      <span className="px-3 text-caption uppercase tracking-wider text-ink-subtle font-medium shrink-0">
                        {t('auth.orEmail')}
                      </span>
                      <div className="border-t border-line w-full" />
                    </div>
                        ...
                        <div>
                          ...
                          <div className="relative">
                            ...
                            <input
                              ...
                              className={`w-full ${isRtl ? 'pr-10 pl-3.5 text-right' : 'pl-10 pr-3.5 text-left'} py-2.5 bg-surface border border-line-strong rounded-control text-sm text-ink placeholder-ink-subtle focus:outline-none focus:border-accent focus:ring-1 focus:ring-accent transition-colors`}
                            />
                          </div>
                        </div>
                      )}
                    ...
                    <div>
                      <button
                        ...
                        className="w-full flex items-center justify-center gap-2.5 px-4 py-2.5 bg-transparent hover:bg-surface-hover text-ink font-medium text-xs sm:text-sm rounded-control border border-line transition-colors active:scale-[0.98] disabled:opacity-50 cursor-pointer"
                      >
                        {guestLoading ? (
                          <Loader2 className="w-4 h-4 animate-spin" />
                        ) : (
                      ...
                      <p className="mt-2 text-caption text-ink-subtle text-center leading-relaxed">
                        {getGuestCaptionText(t, guestLimits)}
                      </p>
                    </div>
                  </div>
                </div>
            ...
            <section id="features-showcase" className="w-full border-t border-line py-16 sm:py-24 mt-12" dir={isRtl ? 'rtl' : 'ltr'}>
              <div className="max-w-[1160px] mx-auto px-4 sm:px-6 lg:px-8">
                <div className="text-center max-w-3xl mx-auto mb-16" dir={isRtl ? 'rtl' : 'ltr'}>
                  <h2 className="text-3xl sm:text-5xl font-semibold text-ink tracking-tighter mb-4">
                ...
                <div className="mt-10 p-6 sm:p-8 rounded-sheet bg-success-soft/70 border border-success/20 transition-colors" dir={isRtl ? 'rtl' : 'ltr'}>
                  <h3 className={`flex items-center gap-2.5 sm:gap-3 text-lg sm:text-2xl font-semibold text-ink mb-2 ${isRtl ? 'text-right' : 'text-left'}`}>
                    <Heart className="w-5 h-5 sm:w-6 sm:h-6 text-success fill-success/20 shrink-0" />
                    <span>{t('auth.freeNonProfitTitle')}</span>
                  </h3>
                  <p className={`text-sm sm:text-base md:text-lg text-ink-muted leading-relaxed ${isRtl ? 'text-right' : 'text-left'}`}>
        ...
        <footer className="public-footer" dir="ltr" style={{ direction: 'ltr' }}>
          <div className="flex items-center gap-2">
            <ChroniXLogo size="sm" variant={isDark ? 'dark' : 'light'} />
            <span>&copy; 2026 ChroniX</span>
          </div>
          ...
          <div className="public-footer-links">
            ...
            <span>&bull;</span>
            <a href="mailto:chronixaicom@gmail.com">
