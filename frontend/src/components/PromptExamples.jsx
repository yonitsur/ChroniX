          ...
          {triplicatedExamples.map((item, idx) => {
            ...
            return (
              <button
                ...
                title={item.prompt}
                dir={isRTL ? 'rtl' : 'ltr'}
                className={`group/card relative flex flex-col justify-between text-start p-2.5 rounded-panel border transition-all duration-200 ease-out cursor-pointer ${
                  isRTL ? 'text-right' : 'text-left'
                } w-52 sm:w-56 h-24 shrink-0 opacity-70 hover:opacity-100 hover:scale-105 hover:z-10 bg-surface-raised/75 hover:bg-surface-raised border-line/75 hover:border-line-strong shadow-control/40 hover:shadow-card disabled:opacity-30 disabled:hover:scale-100 disabled:hover:opacity-30 disabled:cursor-not-allowed`}
              >
                {/* Prompt Title */}
                <p
                  className={`text-xs sm:text-[13px] text-ink group-hover/card:text-accent leading-snug font-medium transition-colors font-sans line-clamp-3 sm:line-clamp-4 ${
