...
export default function PromptExamples({ onSelectPrompt, isGenerating = false }) {
  ...
  const handleManualScroll = (direction) => {
    ...
    const scrollAmount = typeof window !== 'undefined' && window.innerWidth >= 640 ? 236 : 216; // one card width + gap
    el.scrollBy({
          ...
          {triplicatedExamples.map((item, idx) => {
            ...
            return (
              <button
                ...
                title={item.prompt}
                dir={isRTL ? 'rtl' : 'ltr'}
                className={`group/card relative flex flex-col justify-between p-2.5 rounded-panel border transition-all duration-200 ease-out cursor-pointer ${
                  isRTL ? 'text-right' : 'text-left'
                } w-52 sm:w-56 h-24 shrink-0 opacity-70 hover:opacity-100 hover:scale-105 hover:z-10 bg-surface-raised/75 hover:bg-surface-raised border-line/75 hover:border-line-strong shadow-control/40 hover:shadow-card disabled:opacity-30 disabled:hover:scale-100 disabled:hover:opacity-30 disabled:cursor-default`}
              >
                {/* Prompt Title */}
                <p
                  dir={isRTL ? 'rtl' : 'ltr'}
                  className={`text-xs sm:text-body-sm text-ink group-hover/card:text-accent leading-snug font-medium transition-colors font-sans line-clamp-3 sm:line-clamp-4 ${
                    item.isParallel ? 'pe-6' : ''
                  } ${isRTL ? 'text-right' : 'text-left'}`}
                >
                  {item.prompt}
                </p>

                {/* Parallel Lanes Icon Tag (Icon only: bottom-left for RTL, bottom-right for LTR) */}
                {item.isParallel && (
                  <span
                    title={t('toolbar.parallelLanes')}
                    className={`absolute bottom-2.5 ${
                      isRTL ? 'left-2.5' : 'right-2.5'
                    } p-1 rounded border bg-surface-sunken/80 text-accent border-line/70 flex items-center justify-center group-hover/card:border-line-strong transition-colors`}
                  >
                    <Split className="w-3 h-3 shrink-0 text-accent" />
                  </span>
                )}
              </button>
            );
