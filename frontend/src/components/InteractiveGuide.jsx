...
export default function InteractiveGuide({ isOpen, steps = [], onClose }) {
  ...
  useEffect(() => {
    ...
    if (el && typeof ResizeObserver !== 'undefined') {
      ...
      const container = el.closest?.('.absolute, .fixed');
      if (container && container !== el && container !== el.parentElement) {
  ...
  useLayoutEffect(() => {
    ...
    const preferredPlacement = stepsRef.current[index]?.placement || 'bottom';
    let left;
    let top;

    const fitsRight = rect.left + rect.width + GAP + tt.w <= vw - MARGIN;
    const fitsLeft = rect.left - GAP - tt.w >= MARGIN;
    const fitsTop = rect.top - GAP - tt.h >= MARGIN;
    const fitsBottom = rect.top + rect.height + GAP + tt.h <= vh - MARGIN;

    let effectivePlacement = preferredPlacement;
    if (effectivePlacement === 'right' && !fitsRight) {
      if (fitsLeft) effectivePlacement = 'left';
      else if (fitsBottom) effectivePlacement = 'bottom';
      else if (fitsTop) effectivePlacement = 'top';
    } else if (effectivePlacement === 'left' && !fitsLeft) {
      if (fitsRight) effectivePlacement = 'right';
      else if (fitsBottom) effectivePlacement = 'bottom';
      else if (fitsTop) effectivePlacement = 'top';
    } else if (effectivePlacement === 'bottom' && !fitsBottom) {
      if (fitsTop) effectivePlacement = 'top';
      else if (fitsRight) effectivePlacement = 'right';
      else if (fitsLeft) effectivePlacement = 'left';
    } else if (effectivePlacement === 'top' && !fitsTop) {
  ...
  return (
    <div className="fixed inset-0" style={{ zIndex: 9999 }} aria-live="polite" role="dialog" aria-modal="true">
      ...
      >
        ...
        <div className="mt-2 text-center text-caption font-medium text-ink-subtle">
