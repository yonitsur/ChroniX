...
export default function TriviaTicker({ prompt }) {
  ...
  return (
    <div
      dir={isRtl ? 'rtl' : 'ltr'}
      className="w-full max-w-lg mx-auto mt-3 sm:mt-4 flex items-start justify-center gap-2 px-4 min-h-[2.5rem] sm:min-h-[2.25rem]"
    >
      <Lightbulb className="w-4 h-4 text-accent shrink-0 mt-0.5" />
      <p
        className={`text-xs sm:text-sm text-zinc-700 dark:text-ink-muted font-normal sm:font-medium leading-relaxed text-start transition-opacity ease-out ${
