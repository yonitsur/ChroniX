...
function resolveArticleDisplayColor(art, artIdx, isDark, timelineData, themeCategories = [], colorByTheme = false) {
  ...
  if (colorByTheme && cat) {
    return getCategoryColor(cat, themeCategories, isDark);
  } else if (timelineData?.lanes && timelineData.lanes.length > 1) {
    const raw = resolveArticleLaneColor(art?.lane ?? art?.data?.lane, timelineData.lanes, artIdx, false);
    return isDark ? getTimelineDisplayColor(raw, true) : raw;
  } else if (cat) {
    return getCategoryColor(cat, themeCategories, isDark);
  } else {
...
function updateTimelineTheme(timeline, isDark, timelineData) {
  ...
  if (Array.isArray(timeline.lanes)) {
    const bodyBg = isDark ? 'rgba(28, 28, 32, 0.55)' : 'rgba(240, 240, 242, 0.6)';
    const bodyBorder = isDark ? 'rgba(60, 60, 68, 0.7)' : 'rgba(228, 228, 231, 0.8)';
  ...
  if (Array.isArray(timeline.articles) && timelineData) {
    ...
    const themeCategories = isSingleTimeline ? getDistinctCategories(timelineData.articles || []) : [];
...
const TimelineView = forwardRef(({
  ...
  useEffect(() => {
    const selectedIds = activeFilter?.items?.map((item) => String(typeof item === 'object' ? item.id : item)) || [];
    const selectedItems = filterItems.filter((item) => selectedIds.includes(String(item.id)));
  ...
  const handleFilterSelect = (item) => {
    const nextItems = item
      ? activeFiltersRef.current.some((selected) => String(selected.id) === String(item.id))
        ? activeFiltersRef.current.filter((selected) => String(selected.id) !== String(item.id))
        : [...activeFiltersRef.current, item]
      : [];
    activeFiltersRef.current = nextItems;
    setActiveFilterIds(nextItems.map((selected) => String(selected.id)));
  };
  ...
  useImperativeHandle(ref, () => ({
    ...
    setLegendFilter: (itemId) => {
      const item = itemId != null ? (filterItems.find((f) => String(f.id) === String(itemId)) || null) : null;
  ...
  useEffect(() => {
    ...
    const items = filterItems.filter((item) => activeFilterIds.some((id) => String(id) === String(item.id)));
