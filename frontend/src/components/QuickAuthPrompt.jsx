...
export default function QuickAuthPrompt({ onOpenAuth, disabled = false }) {
  ...
  useEffect(() => {
    // Only show for guest users who haven't dismissed it in this session, and when not disabled
    if (!isGuest || disabled) {
      setIsVisible(false);
      return;
    }
    ...
    try {
      const dismissed = sessionStorage.getItem('chronix_quick_auth_dismissed');
      if (dismissed) return;
    } catch {
  ...
  const handleDismiss = () => {
    setIsVisible(false);
    try {
      sessionStorage.setItem('chronix_quick_auth_dismissed', '1');
    } catch {}
  };
    ...
    >
      ...
      <div className="flex items-center gap-2 mb-2 pe-6">
        ...
        <div className="flex items-center gap-1.5 text-xs font-bold text-ink">
          ...
          <span>{t('auth.quickAuthTitle') || (isRtl ? 'התחברות מהירה' : 'Quick Sign In')}</span>
        </div>
      </div>

      {/* Subtitle */}
      <p className="text-xs text-ink-muted mb-3.5 leading-relaxed">
        {t('auth.quickAuthDesc') ||
          (isRtl
            ? 'התחברו בלחיצה כדי לשמור את צירי הזמן שלכם ולסנכרן בין מכשירים.'
            : 'Sign in with one click to keep your timelines and sync across your devices.')}
      </p>
      ...
      <div className="text-center">
        ...
        >
          {t('auth.quickAuthMore') || (isRtl ? 'אפשרויות כניסה נוספות' : 'More sign-in options')}
        </button>
      </div>
    </aside>
  );
