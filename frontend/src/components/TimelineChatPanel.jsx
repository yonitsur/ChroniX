...
export default function TimelineChatPanel({
  ...
  }, [isOpen, minimized, titleText, size.w, vw]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <h3
            className="text-[13px] font-bold text-ink leading-tight m-0 p-0"
            title={titleText}
          >
            <span
              ref={titleTextRef}
      ...
      <div className="border-t border-line p-2.5 shrink-0 bg-surface-sunken">
        {pendingFiles.length > 0 && (
          <div className="flex flex-wrap gap-1 mb-1.5">
            {pendingFiles.map((f, idx) => (
              <span
                key={`${f.name}-${idx}`}
                className="flex items-center gap-1 text-[10.5px] text-ink-muted bg-surface-raised border border-line rounded-control px-1.5 py-0.5 max-w-[140px]"
              >
        ...
        {attachError && (
          <p className="text-[10.5px] text-danger mb-1.5">{attachError}</p>
        )}
        <div className="flex items-end gap-2">
          ...
          <button
            ...
            className="p-2.5 rounded-control bg-surface-raised border border-line hover:bg-surface-hover text-ink-muted hover:text-ink transition-colors cursor-pointer shrink-0 disabled:opacity-40 disabled:cursor-not-allowed shadow-control"
          >
          ...
          <textarea
            ...
            className="flex-1 min-w-0 resize-none max-h-32 bg-surface-raised border border-line rounded-control px-3 py-2 text-[13px] text-ink placeholder-ink-faint outline-none focus:border-accent focus:ring-1 focus:ring-accent-ring/30 transition-all shadow-control scrollbar-none overflow-y-auto leading-normal"
            style={{ minHeight: 40 }}
          />
          {isBusy ? (
            <button
