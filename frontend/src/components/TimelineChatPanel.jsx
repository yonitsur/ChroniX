...
const YEAR_BAR_CLEARANCE_MOBILE = 76;
const YEAR_BAR_CLEARANCE_DESKTOP = 54;
...
export default function TimelineChatPanel({
  ...
  const isNarrow = typeof isMobile === 'boolean' ? isMobile : vw < 768;
  ...
  const touchSwipeStartYRef = useRef(0);
  const handleSwipeTouchStart = (e) => {
    if (e.touches && e.touches[0]) {
      touchSwipeStartYRef.current = e.touches[0].clientY;
    }
  };
  const handleSwipeTouchEnd = (e) => {
    if (e.changedTouches && e.changedTouches[0]) {
      const deltaY = e.changedTouches[0].clientY - touchSwipeStartYRef.current;
      if (deltaY > 50) {
        handleAnimatedClose();
      }
    }
    touchSwipeStartYRef.current = 0;
  };
  ...
  const visibleBubblePos = pos
    ? {
      left: pos.left,
      top: Math.min(pos.top, vh - LAUNCHER_SIZE - yearBarClearance),
    }
    : defaultAnchor;
  const bubbleStyle = pos
    ? { left: `${visibleBubblePos.left}px`, top: `${visibleBubblePos.top}px`, right: 'auto', bottom: 'auto', zIndex }
  ...
  if (!isOpen) {
    ...
    return (
      <button
        ...
        className={`chronix-chat-launcher fixed w-12 h-12 rounded-full flex items-center justify-center cursor-grab active:cursor-grabbing active:scale-95 touch-none overflow-visible shadow-control ${attention ? 'chronix-chat-attention' : ''}`}
      >
  ...
  const activeBubblePos = visibleBubblePos;
  const maxAvailableH = Math.max(220, vh - yearBarClearance - 16);
  const panelW = isNarrow ? narrowFloatW : Math.min(size.w, vw - 16);
  const panelH = isNarrow ? narrowFloatH : Math.min(size.h, maxAvailableH);
  ...
  return (
    <>
      {isNarrow && (
        <div
          className={`fixed inset-0 bg-black/60 backdrop-blur-xs z-40 md:hidden transition-opacity duration-250 ease-out ${isClosing ? 'opacity-0 pointer-events-none' : 'opacity-100 pointer-events-auto'
            }`}
      ...
      <div
        ref={dragRef}
        id="guide-chat-panel"
        style={{ ...panelStyle, maxHeight: isNarrow ? '88vh' : 'calc(100vh - 16px)', maxWidth: isNarrow ? '100vw' : 'calc(100vw - 16px)' }}
        dir={isRtl ? 'rtl' : 'ltr'}
        role="dialog"
        aria-label={timelineTitle || t('chat.title')}
        className={`fixed flex flex-col font-sans z-50 ${isNarrow
            ? `inset-x-0 bottom-0 top-auto h-[82vh] rounded-t-sheet border-t border-line bg-surface-raised shadow-panel transition-transform duration-250 ease-out ${isClosing ? 'translate-y-full' : 'translate-y-0'
            }`
            : 'rounded-sheet border border-line bg-surface-raised shadow-panel overflow-hidden fade-in animate-in duration-200'
          }`}
      >
        {isNarrow && (
          <div
            className="w-full flex justify-center pt-2.5 pb-1 cursor-grab active:cursor-grabbing touch-none select-none md:hidden"
        ...
        >
          ...
          <div
            ...
            className={`min-w-0 flex-1 overflow-hidden select-none px-1 ${titleOverflow > 0 ? 'chat-title-ticker-container' : ''
          ...
          >
            ...
            >
              <span
                ...
                className={`inline-block whitespace-nowrap ${titleOverflow > 0
                    ? isTitleRtl
          ...
          <div
            ...
            dir={isRtl ? 'rtl' : 'ltr'}
            className="seg-track inline-flex items-center p-0.5 rounded-full shrink-0 gap-0.5 select-none"
          >
            <button
              type="button"
              role="radio"
              ...
              className={`seg-item px-2.5 py-1 min-h-[26px] rounded-full text-caption font-semibold whitespace-nowrap cursor-pointer ${grounding ? 'seg-item-active' : ''
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto overscroll-contain px-3 py-3 space-y-3">
          {messages.length === 0 && (
            <div className="flex flex-col items-center justify-center min-h-full py-2 text-center px-3 gap-3">
              ...
              {[
                { key: 'ask', Icon: MessageCircleQuestion, heading: t('chat.suggest.askHeading'), items: suggestions?.questions?.slice(0, 2) },
                { key: 'edit', Icon: PenLine, heading: t('chat.suggest.editHeading'), items: suggestions?.edits?.slice(0, 3) },
              ].map(({ key, Icon, heading, items }) => (items?.length ? (
                <div key={key} className="w-full max-w-[300px] text-start">
                  <p className="flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-wider text-ink-subtle mb-1.5">
                  ...
                  <div className="flex flex-col gap-1.5">
                    {items.map((text) => (
                      <button
                        ...
                        className="w-full text-start text-caption leading-snug text-ink bg-surface-raised hover:bg-surface-hover border border-line rounded-control px-2.5 py-2 transition-colors cursor-pointer"
                      >
                        {text}
                      </button>
                    ))}
              ...
              <div key={m.id} className={`flex ${mine ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[85%] ${mine ? 'items-end' : 'items-start'} flex flex-col gap-1`}>
                  <div
                    ...
                    className={`px-3 py-2 rounded-panel text-body-sm leading-relaxed break-words ${mine
        ...
        <div
          style={{
            paddingBottom: isNarrow ? 'max(14px, calc(env(safe-area-inset-bottom, 12px) + 8px))' : undefined,
          }}
          className="border-t border-line p-2.5 shrink-0 bg-surface-sunken"
        >
          ...
          <div className="flex items-end gap-2">
            ...
            <textarea
              ...
              className={`flex-1 min-w-0 resize-none max-h-32 bg-surface-raised border border-line rounded-control px-3 py-2 text-body-sm text-ink placeholder-ink-faint placeholder:truncate outline-none focus:border-accent focus:ring-1 focus:ring-accent-ring/30 transition-all shadow-control scrollbar-none overflow-y-auto leading-normal ${(isRtlText(input) || (!input && isRtl)) ? 'text-right' : 'text-left'
                }`}
              style={{ minHeight: 40 }}
            />
            {isBusy ? (
