...
export default function GeoMapView({
  ...
  useEffect(() => {
    ...
    let initialZoom = Math.max(initialMinZoom, 2);
    ...
    if (savedState && savedState.center && savedState.zoom) {
      ...
      initialZoom = Math.max(savedState.zoom, initialMinZoom);
    ...
    sortedGeoArticles.forEach((art, idx) => {
      ...
      const popupHtml = `
        <div class="chronix-popup-card chronix-popup-clickable p-3 max-w-[260px] text-ink font-sans ${popupAlign} cursor-pointer select-none" dir="${popupDir}" data-article-id="${art.id}">
          ...
          ${
            art.subtitle
              ? `<p class="text-caption text-ink-muted leading-tight mb-2 line-clamp-2">${art.subtitle}</p>`
              : ''
          }
          ${
            art.locationName
              ? `<div class="flex items-center gap-1.5 text-caption text-ink-muted font-medium mb-1.5">
          ...
          ${
            badgeLabel
              ? `<div class="flex items-center gap-1.5 mb-2.5">
                  ...
                  <span class="text-caption font-medium text-ink-subtle truncate">${badgeLabel}</span>
                </div>`
              : ''
          }
          <div class="flex items-center gap-2 pt-2 border-t border-line">
            <button type="button" data-action="details" class="w-full flex items-center justify-center gap-1 py-1.5 px-2 text-caption font-semibold rounded-control bg-accent-soft hover:bg-accent-soft/80 text-accent border border-accent/40 transition-colors cursor-pointer">
      ...
      marker.on('popupopen', () => {
        ...
        if (isMobileRef.current && onRequestMapHeightRef.current) {
          requestAnimationFrame(() => {
            ...
            const grew = onRequestMapHeightRef.current(popupEl.offsetHeight + PIN_AND_MARGIN);
  ...
  }, [filteredGeoArticles, laneColorMap, categories, selectedArticleId, isMobile, isExploring, isRtl]);
  ...
  // 4b. Location-less event during the mobile guided tour: identify if active event has no coordinates
  const nonGeoActiveArticle = useMemo(() => {
    if (!isMobile || !isExploring || !selectedArticleId || isCenterPopupDismissed) return null;
    const art = articles.find((a) => a.id === selectedArticleId);
    return (art && !geoArticles.some((g) => g.id === art.id)) ? art : null;
  }, [isMobile, isExploring, selectedArticleId, isCenterPopupDismissed, articles, geoArticles]);
  ...
  return (
    <div className={`relative w-full h-full overflow-hidden select-none bg-surface-sunken ${className}`}>
      ...
      {/* Backdrop blur overlay & Centered Card for Non-Geographic Events on Mobile */}
      {nonGeoActiveArticle && (() => {
        ...
        const yearStr = nonGeoActiveArticle.from ? formatDatePart(nonGeoActiveArticle.from) : '';
        ...
        return (
          <div
            className="absolute inset-0 z-20 flex items-end justify-center pb-4 px-3 bg-black/25 dark:bg-black/45 backdrop-blur-[2px] transition-all duration-300 pointer-events-auto"
            onClick={() => setIsCenterPopupDismissed(true)}
          >
            <div
              onClick={(e) => {
                e.stopPropagation();
              ...
              dir={popupDir}
              className={`relative w-full max-w-[280px] bg-surface-overlay text-ink border border-warning shadow-[0_0_0_1px_rgb(var(--warning)),0_16px_32px_-4px_rgba(0,0,0,0.42)] rounded-panel p-3 ${popupAlign} cursor-pointer select-none animate-in fade-in slide-in-from-bottom-2 duration-200`}
            >
              {/* Close button */}
              <button
                type="button"
                ...
                className="absolute top-2.5 right-2.5 rtl:right-auto rtl:left-2.5 p-1 text-ink-subtle hover:text-ink transition-colors cursor-pointer rounded-control"
              ...
              <div className="flex items-center gap-1 text-caption font-semibold uppercase tracking-wider text-warning mb-1.5 pr-6 rtl:pr-0 rtl:pl-6">
                <MapPinOff className="w-3 h-3 shrink-0" />
                <span>{t('floatingMap.noLocation')}</span>
              </div>

              {/* Image if present */}
              {nonGeoActiveArticle.imageUrl && (
                <div className="w-full h-24 mb-2 rounded-control overflow-hidden bg-surface-sunken border border-line">
                  <img
                    src={nonGeoActiveArticle.imageUrl}
                    alt={nonGeoActiveArticle.title}
              ...
              {/* Event number & Date */}
              {(eventNum || yearStr) && (
                <div className="flex items-center gap-1.5 text-caption font-semibold text-accent uppercase tracking-wider mb-0.5">
                  {eventNum && <span>#{eventNum}</span>}
                  {eventNum && yearStr && <span>•</span>}
                  {yearStr && <span>{yearStr}</span>}
                </div>
              )}

              {/* Title & Subtitle */}
              <h3 className="text-sm font-bold leading-snug mb-1 text-ink">
                {nonGeoActiveArticle.title}
              </h3>
              {nonGeoActiveArticle.subtitle && (
                <p className="text-caption text-ink-muted leading-tight mb-2 line-clamp-2">
                  {nonGeoActiveArticle.subtitle}
                </p>
              )}
              ...
              {badgeLabel && (
                <div className="flex items-center gap-1.5 mb-1">
                  ...
                  />
                  <span className="text-caption font-medium text-ink-subtle truncate">
                    {badgeLabel}
                  </span>
                </div>
              )}
              ...
              {nonGeoActiveArticle.wikiUrl && (
                <div className="flex items-center gap-2 pt-2 mt-1 border-t border-line">
                  <a
                    ...
                    className="flex-1 flex items-center justify-center gap-1.5 py-1.5 px-2.5 text-caption font-medium rounded-control bg-surface-raised hover:bg-surface-hover text-ink hover:text-accent border border-line transition-colors cursor-pointer shadow-control group"
                  >
                    <span className="truncate">{t('eventDrawer.wikiLink')}</span>
              ...
              <button
                ...
                className="w-full mt-2 flex items-center justify-center gap-1 py-1.5 px-2 text-caption font-semibold rounded-control bg-accent-soft hover:bg-accent-soft/80 text-accent border border-accent/40 transition-colors cursor-pointer"
              >
                <span>{t('eventDrawer.title')}</span>
      ...
      <div className="absolute top-4 left-4 z-20 flex items-center gap-2">
        <button
          ...
          className="flex items-center gap-2 px-3 py-1.5 rounded-control text-xs font-medium bg-surface-raised border border-line shadow-panel text-ink hover:bg-surface-hover hover:text-accent transition-colors cursor-pointer"
