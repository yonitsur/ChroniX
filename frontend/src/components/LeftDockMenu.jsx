...
export default function LeftDockMenu({
  ...
  const handleToggleCommunityPublish = async () => {
    if (isPublishing) return;
    setIsPublishing(true);
    setShareError(null);
    try {
      const author = publishAnonymous
        ? 'Community Explorer'
        : (user && !isGuest
            ? (user?.user_metadata?.full_name || user?.user_metadata?.name || user?.email?.split('@')[0] || 'Community Explorer')
            : 'Guest Explorer');
      await onToggleShare?.(author);
      if (!isShared) {
        await resolveShareLink();
      }
    } catch (err) {
      setShareError(err?.message || 'Failed to update community wall status');
    } finally {
    ...
    <>
      ...
      {canShare && (
        <div>
          ...
          {isShareSubmenuOpen && (
            <div className="mx-2 mb-2 p-1.5 bg-surface-sunken/60 rounded-control border border-line flex flex-col gap-1.5 animate-in fade-in zoom-in-95 duration-100">
              {shareError && (
                <p className="px-2.5 py-1 text-caption text-danger">{shareError}</p>
              )}

              {/* Community Wall Publication Card */}
              <div className="p-2 rounded-lg bg-surface-raised border border-line flex flex-col gap-2">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1.5">
                    <Globe className={`w-3.5 h-3.5 ${isShared ? 'text-emerald-500' : 'text-ink-subtle'}`} />
                    <span className="text-xs font-semibold text-ink">
                      {isShared
                        ? (t('community.publishedToWall') || 'Public on Wall')
                        : (t('community.privateOnly') || 'Private Timeline')}
                    </span>
                  </div>
                  <span
                    className={`text-[9px] font-bold px-1.5 py-0.5 rounded tracking-wide ${
                      isShared
                        ...
                        : 'bg-surface-sunken text-ink-subtle border border-line/60'
                    }`}
                  >
                ...
                <p className="text-[11px] text-ink-muted leading-relaxed">
                  {isShared
                    ? (t('toolbar.publishedToWallNotice') || 'Your timeline is published on the Community Wall for everyone to explore!')
                    : (t('community.encourageSharePrompt') || 'Timelines are private by default. Publish to the Community Wall to share it!')}
                </p>
                ...
                {!isShared && (
                  <label className="flex items-center gap-1.5 text-[11px] text-ink-subtle cursor-pointer select-none">
                    <input
                      ...
                      className="rounded border-line text-accent focus:ring-0 cursor-pointer"
                    />
                    <span>{t('community.anonymousOption') || 'Publish anonymously'}</span>
                  </label>
                )}

                <button
                  type="button"
                  onClick={handleToggleCommunityPublish}
                  disabled={isPublishing}
                  className={`w-full py-1.5 px-2.5 rounded-control text-xs font-semibold flex items-center justify-center gap-1.5 transition-all cursor-pointer ${
                    isShared
                      ? 'bg-surface-sunken hover:bg-surface-hover text-ink-muted hover:text-ink border border-line'
                      : 'bg-accent hover:bg-accent-hover text-accent-fg shadow-control'
                  }`}
                >
                  {isPublishing ? (
                    <Loader2 className="w-3.5 h-3.5 animate-spin" />
                  ) : isShared ? (
                    <span>{t('community.unpublishFromWall') || 'Unpublish from Wall'}</span>
                  ) : (
                    <>
              ...
              {shareLoading && (
                <div className="flex items-center gap-2 px-2.5 py-1 text-xs text-ink-subtle">
              ...
              {shareLink && (
                <div className="flex flex-col gap-0.5 pt-1 border-t border-line/50">
                  <button
                    type="button"
                  ...
                  >
                    <span className="flex items-center gap-2 truncate">
                      ...
                      <span className="truncate">
                        {isLinkCopied
                          ? (t('exportModal.copiedNotification') || 'Copied!')
                          : (t('common.copyLink') || 'Copy Share Link')}
                      </span>
                    </span>
      ...
      {/* About & Disclaimer (In-App Modals) */}
      <button
        type="button"
        onClick={() => {
          setOpenMenu(null);
          ...
          {items.map((item) => {
            ...
            if (isEditingThis) {
              return (
                <div
                  key={item.id}
                  className="w-full px-1.5 py-1 rounded-control bg-surface-sunken/90 border border-accent/60 shadow-xs animate-in fade-in duration-100"
                ...
                >
                  ...
                  >
                    ...
                    <input
                      ...
                      className="flex-1 min-w-0 px-1.5 py-0.5 text-xs bg-surface text-ink border border-line rounded focus:outline-none focus:border-accent font-normal"
                      maxLength={60}
                    ...
                    <button
                      ...
                      className="p-1 rounded bg-accent text-accent-fg hover:bg-accent-hover transition-colors disabled:opacity-30 disabled:cursor-default shrink-0 cursor-pointer"
                    ...
                    <button
                      ...
                      className="p-1 rounded text-ink-subtle hover:text-ink hover:bg-surface-overlay transition-colors shrink-0 cursor-pointer"
            ...
            return (
              <div
                key={item.id}
                className={`group relative flex items-center justify-between w-full px-2 py-1 text-xs rounded-control transition-colors ${
                  isActive
                    ? 'bg-accent-soft text-accent font-semibold'
                    : 'text-ink-muted hover:bg-surface-hover hover:text-ink'
                }`}
                ...
                <div className="flex items-center gap-1 shrink-0 ms-1.5">
                  ...
                  {canEdit && (
                    <button
                      ...
                      title={dimension === 'lane' ? (t('toolbar.editLaneName') || t('common.edit') || 'עריכת שם המסלול') : (t('toolbar.editThemeName') || t('common.edit') || 'עריכת שם הנושא')}
                      aria-label={dimension === 'lane' ? (t('toolbar.editLaneName') || t('common.edit') || 'עריכת שם המסלול') : (t('toolbar.editThemeName') || t('common.edit') || 'עריכת שם הנושא')}
                    >
    ...
    <div ref={rootRef}>
      ...
      >
        ...
        <div className="flex flex-col items-center gap-1.5 w-full px-1.5">
          ...
          {/* 6.5 גלובוס - Quick 1-Click Community Wall toggle */}
          {canShare && (
            <button
              id={!isMobile ? 'guide-dock-community-share' : undefined}
              type="button"
              onClick={handleToggleCommunityPublish}
              disabled={isPublishing}
              className={`${railBtn} relative ${
                isShared
                  ...
                  : 'text-ink-subtle hover:text-accent hover:bg-accent/10'
              }`}
              title={
                isShared
                  ? (t('community.publishedToWallTooltip') || 'מוצג בלוח הקהילה - לחץ להפיכה לפרטי')
                  : (t('community.publishToWallTooltip') || 'פרסם לקיר הקהילה (קליק אחד)')
              }
              aria-label={isShared ? t('community.unpublishFromWall') : t('community.publishToWall')}
              aria-pressed={isShared}
            >
              {isPublishing ? (
                <Loader2 className="w-5 h-5 shrink-0 animate-spin text-accent" />
              ) : (
                <Globe className="w-5 h-5 shrink-0" strokeWidth={1.8} />
              )}
              {isShared && (
                <span className="absolute top-1 right-1 w-2 h-2 rounded-full bg-emerald-500 ring-2 ring-surface shadow-xs" />
              )}
            </button>
          )}
      ...
      >
        ...
        {mobileCollapsed ? (
          ...
          <div className="relative flex flex-col items-center gap-1 p-1 rounded-2xl bg-surface-overlay/95 dark:bg-surface-raised/90 backdrop-blur-xl border border-line/70 shadow-pop animate-in fade-in zoom-in-95 duration-150">
            ...
            {/* Quick 1-Click Community Wall toggle */}
            {canShare && (
              <button
                id={isMobile ? 'guide-dock-community-share' : undefined}
                type="button"
