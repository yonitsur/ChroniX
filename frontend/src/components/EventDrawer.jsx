...
import { X, ExternalLink, Edit, Trash2, Calendar, Layers, Tag, Image as ImageIcon, AlertTriangle, MapPin, Star, ChevronLeft, ChevronRight, Compass, Globe, MessageSquare, Maximize2, PanelLeftClose, Minus, Plus, Move, Check, Quote, Images, Play, Pause, Pencil } from 'lucide-react';
...
export function formatDatePart(d, lang = 'en') {
  ...
  if (d.month && d.day) {
    ...
    return fmt.replace('{day}', d.day).replace('{month}', monthName).replace('{year}', y);
  }
...
export default function EventDrawer({
  ...
  return (
    <>
      ...
      <div
        className={`fixed md:absolute inset-x-0 bottom-0 md:inset-y-0 md:left-[52px] md:right-auto w-full md:w-[380px] lg:w-[440px] md:max-w-[calc(100vw-56px)] h-[86vh] md:h-full max-h-[90vh] md:max-h-full rounded-t-3xl md:rounded-none bg-surface-overlay border-t md:border-t-0 md:border-r border-line shadow-panel flex flex-col transition-all duration-300 ease-in-out font-sans z-50 ${
          isClosing
      ...
      >
        ...
        <div
          ...
          className={`space-y-1.5 ${isTitleRtl ? 'text-right' : (isRtl ? 'text-right' : 'text-left')}`}
        >
        ...
        {(timeSpan || themeLabel || showLaneBadge || isEdited || article.locationName || (article.lat != null && article.lng != null)) && (
          <div className="space-y-2.5">
            ...
            <div className="flex flex-wrap items-center gap-1.5 text-xs">
              ...
              {/* Topic chip */}
              {themeLabel && (
                <span className="inline-flex items-center gap-1.5 font-medium text-ink bg-surface-raised border border-line px-3 py-1 rounded-full">
                  <span
                    className="w-2.5 h-2.5 rounded-full shrink-0 ring-1 ring-black/10 dark:ring-white/20"
                    style={{ backgroundColor: themeColor }}
                  />
                  <span dir={isThemeRtl ? 'rtl' : (isRtl ? 'rtl' : 'ltr')} className="truncate max-w-[170px]">
                    {themeLabel}
                  </span>
                </span>
              )}
              ...
              {/* Lane chip */}
              {showLaneBadge && (
                <span className="inline-flex items-center gap-1.5 font-medium text-ink-muted bg-surface-raised border border-line px-3 py-1 rounded-full">
                  <Layers className="w-3.5 h-3.5 text-ink-subtle shrink-0" />
                  <span dir={isLaneRtl ? 'rtl' : (isRtl ? 'rtl' : 'ltr')} className="truncate max-w-[150px]">
                    {laneObj.title}
                  </span>
                </span>
              )}
            ...
            {(article.locationName || (article.lat != null && article.lng != null)) ? (
              <div className="flex items-center justify-between gap-2 p-2.5 rounded-xl bg-surface-hover/40 border border-line text-xs">
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <span className="leading-snug truncate font-medium text-ink">
              ...
              {additionalImages.map((img, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    {img.caption && (
                      <div
                        className="px-2 py-1 text-caption text-ink-muted truncate w-full"
