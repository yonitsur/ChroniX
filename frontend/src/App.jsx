...
import { FolderOpen, AlertTriangle, Loader2, MapPin, Maximize2, Minimize2, X, Columns2, BookUser, Dices, Plus, ShieldCheck, Eye } from 'lucide-react';
...
export default function App() {
  ...
  // Enables sharing (first click) if needed and returns the shareable "/t/<id>" URL.
  const handleGetShareLink = async () => {
    if (!currentTimeline?.id) throw new Error('No timeline to share yet');
    if (!currentTimeline.isShared) {
  ...
  if (!user && isSharedView && (isSharedViewLoading || (!currentTimeline && !sharedViewError))) {
    return (
      <div className="min-h-screen w-full flex flex-col items-center justify-center bg-surface text-ink gap-3 select-none">
        <div className="w-12 h-12 rounded-panel bg-accent-soft border border-line flex items-center justify-center">
          <Loader2 className="w-6 h-6 animate-spin text-accent" />
  ...
  if (!user && isSharedView && sharedViewError) {
    return <AuthGate theme={theme} onToggleTheme={handleToggleTheme} />;
  }

  // Read-only whenever this timeline was loaded from a share link and the viewer isn't its owner
  // (covers logged-out visitors, guests, and other registered users alike).
  const isReadOnlyView = isSharedView && currentTimeline?.isOwner === false;
        ...
        <div className="absolute top-3 left-1/2 -translate-x-1/2 z-30 bg-surface-overlay border border-line text-ink-muted px-3.5 py-2 rounded-panel text-xs flex flex-wrap items-center justify-center gap-2.5 shadow-card max-w-xl text-center">
          ...
          {user && !isGuest ? (
            <button
              ...
              className="px-2.5 py-1 rounded-control bg-accent hover:bg-accent-hover text-accent-fg font-semibold text-[11.5px] transition-colors whitespace-nowrap shadow-control cursor-pointer"
            >
      ...
      <main className="flex-1 min-h-0 relative w-full overflow-hidden">
        ...
        {currentTimeline ? (
          <div
            ...
            className={`w-full h-full flex flex-col overflow-hidden relative md:pl-[52px] ${
          ...
          <div className="w-full h-full overflow-y-auto overscroll-y-contain flex flex-col items-center text-center justify-between sm:justify-center p-3 sm:p-6 pb-6 sm:pb-20 select-none animate-in fade-in duration-300 bg-transparent">
            <div className="w-full max-w-4xl mx-auto my-auto flex flex-col items-center justify-center">
              ...
              <div className="pb-1 flex flex-wrap items-center justify-center gap-1.5 sm:gap-2" dir="ltr" style={{ direction: 'ltr' }}>
                <button
                  type="button"
