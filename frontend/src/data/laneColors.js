...
export const LEGACY_COLOR_MAP = {
  // Legacy default blue -> Archival Basalt Gray (#454b52)
  '#2b5278': '#454b52',

  // Blues -> Aegean Petrol (#24657a)
  '#2563eb': '#24657a',
  '#3b82f6': '#24657a',
  '#1d4ed8': '#24657a',
  '#1e40af': '#24657a',
...
function resolvePreferredLaneColor(lane, index = 0) {
  ...
  // 1. If it's in our legacy color map, translate it to its museum counterpart
  if (LEGACY_COLOR_MAP[lower]) {
    return LEGACY_COLOR_MAP[lower];
  }

  // 2. If it's already one of our curated museum colors, keep it!
...
// Vibrant archival palette for categories & themes (day mode)
...
// Illuminated, high-contrast palette calibrated specifically for Dark Mode
export const DARK_CATEGORY_PALETTE = [
  '#f87171', // Coral Terracotta
  '#38bdf8', // Mineral Teal / Sky
  '#34d399', // Luminous Sage / Emerald
  '#fbbf24', // Golden Ochre / Amber
  '#c084fc', // Vibrant Mulberry / Purple
  '#60a5fa', // Azure Blue / Lapis
  '#fb7185', // Rosewood Crimson
  '#fb923c', // Bronze Apricot / Orange
  '#818cf8', // Periwinkle Indigo
  '#2dd4bf', // Turquoise Mint
  '#e879f9', // Orchid
  '#4ade80', // Emerald Green
];

export const DARK_LANE_COLORS = [
  '#94a3b8', // Slate Silver (Default lane in dark mode)
  '#f87171', // Coral Terracotta
  '#34d399', // Luminous Sage
  '#c084fc', // Vibrant Mulberry
  '#fbbf24', // Golden Ochre
  '#38bdf8', // Mineral Teal
  '#fb923c', // Bronze Apricot
...
/**
 * Maps any lane or category color into an illuminated, high-contrast tone for dark mode.
 */
export function getTimelineDisplayColor(color, isDark = false) {
  if (!isDark || !color) return color;
  const lower = String(color).toLowerCase().trim();

  // 1. Check if it's one of our default lane colors
  const laneIdx = DEFAULT_LANE_COLORS.findIndex((c) => c.toLowerCase() === lower);
  if (laneIdx >= 0) {
    return DARK_LANE_COLORS[laneIdx % DARK_LANE_COLORS.length];
  }

  // 2. Check if it's one of our category palette colors
  const catIdx = CATEGORY_PALETTE.findIndex((c) => c.toLowerCase() === lower);
  if (catIdx >= 0) {
    return DARK_CATEGORY_PALETTE[catIdx % DARK_CATEGORY_PALETTE.length];
  }

  // 3. If it's another hex color, check its luminance and lift if too dark
  if (lower.startsWith('#') && (lower.length === 7 || lower.length === 4)) {
    let clean = lower.slice(1);
    ...
    if (clean.length === 6) {
      ...
      if (lum < 0.38) {
        const mapped = mapHexToMuseumColor(lower);
        if (mapped) {
          const mIdx = DEFAULT_LANE_COLORS.findIndex((c) => c.toLowerCase() === mapped.toLowerCase());
          if (mIdx >= 0) return DARK_LANE_COLORS[mIdx];
        }
      }
    }
  }
 ...
 */
export function getCategoryColor(category, allCategories = [], isDark = false) {
  let key = category;
